...
public class BankAccountServiceImpl implements BankAccountService {
    ...
    public List<Account> getBankAccounts() {
        Company bank = companyRepository.findByName(BANK_COMPANY_NAME).orElseThrow(() -> new CompanyNotFound(BANK_COMPANY_NAME));
    ...
    public Account getBankAccountForCurrency(Currency.Code currency) {
        return getBankAccounts().stream()
                .filter(account -> account.getCurrency().getCode() == currency)
                .findFirst()
                .orElseThrow(() -> new AccountNotFound("Bank account for currency " + currency + " not found"));
    }

    public Page<TransactionDto> getAllTransactionsForBank(Authentication authentication, PageRequest pageRequest) {
        ...
        Company bank = companyRepository.findByName(BANK_COMPANY_NAME)
                .orElseThrow(() -> new CompanyNotFound(BANK_COMPANY_NAME));

        if (!bank.getMajorityOwner().equals(client)) {
