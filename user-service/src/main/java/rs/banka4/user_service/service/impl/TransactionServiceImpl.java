...
public class TransactionServiceImpl implements TransactionService {
    ...
    public TransactionDto createTransaction(Authentication authentication, CreatePaymentDto createPaymentDto) {
        ...
        Transaction transaction = processTransaction(fromAccount, toAccount, createPaymentDto.fromAmount(), createPaymentDto);
    ...
    public TransactionDto createTransfer(Authentication authentication, CreateTransferDto createTransferDto) {
        ...
        Transaction transaction = processTransaction(fromAccount, toAccount, createTransferDto.fromAmount(), createTransferDto);
     ...
     * <p>
     * Handles the transfer of funds between the specified accounts, including any necessary currency conversions
     * and fee calculations. It supports transactions between accounts with the same currency, as well as transactions involving
    ...
    public Transaction processTransaction(Account fromAccount, Account toAccount, BigDecimal amount, CreateTransactionDto createTransactionDto) {
        ...
        // RSD -> Foreign
        else if (fromAccount.getCurrency().getCode().equals(Currency.Code.RSD)) {
            fee = transferFromRsdToForeign(fromAccount, toAccount, amount);
        }
        else {
            // Foreign -> RSD
            if (toAccount.getCurrency().getCode().equals(Currency.Code.RSD)) {
     ...
     * <p>
     * Handles the transfer of funds from an RSD account to a foreign currency account, including fee calculation,
     * currency conversion, and updating the balances of the involved accounts. It ensures that the account has sufficient funds
     * to cover the transfer amount and the associated fee.
     * </p>
     *
     ...
     * @throws InsufficientFunds if the account has insufficient funds to cover the transfer amount and fee
     */
    private BigDecimal transferFromRsdToForeign(Account fromAccount, Account toAccount, BigDecimal amount){
        ...
        Account rsdBankAccount = bankAccountServiceImpl.getBankAccountForCurrency(Currency.Code.RSD);
        fromAccount.setBalance(fromAccount.getBalance().subtract(amount));
        rsdBankAccount.setBalance(rsdBankAccount.getBalance().add(amount));
        createBankTransferTransaction(fromAccount, rsdBankAccount, amount, "Transfer to RSD Bank Account");
        ...
        foreignBankAccount.setBalance(foreignBankAccount.getBalance().subtract(convertedAmount));
     ...
     * <p>
     * Handles the transfer of funds from a foreign currency account to an RSD account, including fee calculation,
     * currency conversion, and updating the balances of the involved accounts. It ensures that the account has sufficient funds
     * to cover the transfer amount and the associated fee.
     * </p>
     *
     ...
     * @throws InsufficientFunds if the account has insufficient funds to cover the transfer amount and fee
     */
    private BigDecimal transferFromForeignToRsd(Account fromAccount, Account toAccount, BigDecimal amount){
        BigDecimal fee = exchangeRateService.calculateFee(amount);
        ...
        Account foreignBankAccount = bankAccountServiceImpl.getBankAccountForCurrency(fromAccount.getCurrency().getCode());
        fromAccount.setBalance(fromAccount.getBalance().subtract(amount));
        foreignBankAccount.setBalance(foreignBankAccount.getBalance().add(amount));
        createBankTransferTransaction(fromAccount, foreignBankAccount, amount, "Transfer to Foreign Bank Account");

        // Convert Foreign to RSD using the buy rate
        BigDecimal convertedAmount = exchangeRateService.convertCurrency(amount, fromAccount.getCurrency().getCode(), Currency.Code.RSD);

        // Transfer RSD from RSD bank account to client
        Account rsdBankAccount = bankAccountServiceImpl.getBankAccountForCurrency(Currency.Code.RSD);
        rsdBankAccount.setBalance(rsdBankAccount.getBalance().subtract(convertedAmount));
        toAccount.setBalance(toAccount.getBalance().add(convertedAmount));
     ...
     * <p>
     * Handles the transfer of funds between two foreign currency accounts, including fee calculation,
     * currency conversion, and updating the balances of the involved accounts. It ensures that the account has sufficient funds
     * to cover the transfer amount and the associated fee.
     * </p>
     *
     ...
     * @throws InsufficientFunds if the account has insufficient funds to cover the transfer amount and fee
     */
    private BigDecimal transferFromForeignToForeign(Account fromAccount, Account toAccount, BigDecimal amount){
        BigDecimal fee = exchangeRateService.calculateFee(amount).multiply(BigDecimal.TWO);
        ...
        // Convert ForeignFrom to RSD using the sell rate
        // EUR Bank -> RSD Bank
        Account rsdBankAccount = bankAccountServiceImpl.getBankAccountForCurrency(Currency.Code.RSD);
        BigDecimal amountInRSD = exchangeRateService.convertCurrency(amount, fromAccount.getCurrency().getCode(), Currency.Code.RSD);
        rsdBankAccount.setBalance(rsdBankAccount.getBalance().add(amountInRSD));
        createBankTransferTransaction(foreignBankAccount, rsdBankAccount, amount, "Transfer to RSD Bank Account");
        ...
        rsdBankAccount.setBalance(rsdBankAccount.getBalance().subtract(amountInRSD));
        BigDecimal amountInForeignTo = exchangeRateService.convertCurrency(amountInRSD, Currency.Code.RSD, toAccount.getCurrency().getCode());
        Account foreignToBankAccount = bankAccountServiceImpl.getBankAccountForCurrency(Currency.Code.USD);
        foreignToBankAccount.setBalance(foreignToBankAccount.getBalance().add(amountInForeignTo));

        // Transfer ForeignTo from ForeignTo bank account to client (USD Bank -> USD Client)
        foreignToBankAccount.setBalance(foreignToBankAccount.getBalance().subtract(amountInForeignTo));
        toAccount.setBalance(toAccount.getBalance().add(amountInForeignTo));
        createBankTransferTransaction(foreignToBankAccount, toAccount, amountInForeignTo, "Transfer converted amount to Client");
    ...
    private void createFeeTransaction(Account fromAccount, Account toAccount, BigDecimal fee) {
        Transaction feeTransaction = buildSpecialTransaction(
                fromAccount, toAccount, fee, fee, "Bank Fee", "289", "FEE-" + UUID.randomUUID(), "Transaction Fee"
        );
        transactionRepository.save(feeTransaction);
    }

    private void createConversionTransaction(Account fromAccount, Account toAccount, BigDecimal amount) {
        Transaction conversionTransaction = buildSpecialTransaction(
                fromAccount, toAccount, amount, BigDecimal.ZERO, "Bank Conversion", "285", "CONV-" + UUID.randomUUID(), "Currency Conversion"
        );
        transactionRepository.save(conversionTransaction);
    }

    private void createBankTransferTransaction(Account fromAccount, Account toAccount, BigDecimal amount, String purpose) {
        Transaction transaction = buildSpecialTransaction(
                fromAccount, toAccount, amount, BigDecimal.ZERO, "Bank Transfer", "290", "TRF-" + UUID.randomUUID(), purpose
        );
        transactionRepository.save(transaction);
    }

    private Transaction buildSpecialTransaction(Account fromAccount, Account toAccount, BigDecimal amount, BigDecimal fee, String recipient, String paymentCode, String referenceNumber, String paymentPurpose) {
        return Transaction.builder()
                .transactionNumber(UUID.randomUUID().toString())
                .fromAccount(fromAccount)
                .toAccount(toAccount)
    ...
    private boolean hasEnoughFunds(Account account, BigDecimal fee) {
        return account.getBalance().subtract(fee).compareTo(BigDecimal.ZERO) >= 0;
    }
