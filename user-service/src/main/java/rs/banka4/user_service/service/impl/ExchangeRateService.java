 ...
 *
 * <p>This service fetches exchange rates from an external service and provides methods
 * for currency conversion and fee calculation.</p>
 */
@Service
...
public class ExchangeRateService {
     ...
     * @throws org.springframework.web.client.RestClientException if the external service is unavailable.
     */
     ...
     * <ul>
     *     <li>If converting from RSD, the amount is divided by the buy rate of the target currency.</li>
     *     <li>If converting to RSD, the amount is multiplied by the sell rate of the source currency.</li>
     *     <li>If converting between two non-RSD currencies, the amount is first converted to RSD using the sell rate,
     *     and then converted to the target currency using the buy rate.</li>
     * </ul>
     *
    ...
    public BigDecimal convertCurrency(BigDecimal amount, Currency.Code from, Currency.Code to) {
        ...
        if (from.equals(Currency.Code.RSD)) {
            return amount.divide(exchangeRates.get(to).buy(), 2, RoundingMode.HALF_UP);
        }
        else if (to.equals(Currency.Code.RSD)) {
            return amount.multiply(exchangeRates.get(from).sell());
        }
    ...
    public BigDecimal calculateFee(BigDecimal amount) {
        return amount.multiply(BANK_FEE).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
    }
}
